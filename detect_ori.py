        ...
        if h1-h2>0 and l1-l2>0:
            ...
            if len(idxs) > 0:
                ...
                for i in idxs.flatten():
                    ...
                    text = "{}: {:.4f}, Size: {}".format(LABELS[classIDs[i]], confidences[i], Size_Class)
                    cv2.putText(img, text, (l1-5, h1 - 5), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, color, 2)

            # show the output image
            #cv2.imshow("Image", img)
            cv2.imwrite("results/"+num + '.jpg', img)
...
print("{:.2f} seconds".format(end_all - start_all))
